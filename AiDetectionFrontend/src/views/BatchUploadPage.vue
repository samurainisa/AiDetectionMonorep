<template>
  <VeritasShell active="batch" :breadcrumbs="[{ label: 'Пакетная загрузка' }]">
    <div style="padding:24px 28px;display:flex;flex-direction:column;gap:20px">
      <div>
        ...
        <p style="font-size:13px;color:var(--muted);margin-top:4px">Загрузите до 50 файлов — проверим параллельно, покажем сводку.</p>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 320px;gap:16px">
        ...
        <div class="va-card" style="padding:0;overflow:hidden;display:flex;flex-direction:column">
          ...
          <div style="padding:14px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between">
            ...
            <div style="display:flex;gap:8px">
              <button class="va-btn ghost" style="height:28px" @click="files=[]">Очистить</button>
              <button class="va-btn accent" style="height:28px" @click="runAll" :disabled="isRunning">
                <VIcon name="zap" :size="12" /> Запустить все
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div v-for="(f, i) in files" :key="i"
              style="display:grid;grid-template-columns:1fr 120px 120px 120px 32px;align-items:center;gap:12px;padding:12px 16px"
              ...
              <div>
                <span v-if="f.result" :class="`va-chip ${verdictFromResult(f.result)}`">
              ...
              <button class="va-btn ghost" style="width:28px;height:28px;padding:0;justify-content:center" @click="files.splice(i,1)">
        ...
        <div style="display:flex;flex-direction:column;gap:12px">
          ...
          <div class="va-card" style="padding:18px">
            <div style="font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;font-weight:600;margin-bottom:10px">Настройки</div>
            <label style="display:flex;align-items:center;gap:8px;font-size:13px;padding:6px 0">
              <input type="checkbox" v-model="checkAI" :style="{ accentColor:'var(--accent)' }" /> Проверка ИИ
            </label>
            <label style="display:flex;align-items:center;gap:8px;font-size:13px;padding:6px 0">
              <input type="checkbox" v-model="checkPlagiarism" :style="{ accentColor:'var(--accent)' }" /> Проверка плагиата
            </label>
          </div>
        </div>
      </div>
...
const verdictFromResult = (result: AnalyzeResponse | null): Verdict =>
  resolveVerdict(result?.pangram_response)

const aiPercentFromResult = (result: AnalyzeResponse | null) =>
  resolveAiPercent(result?.pangram_response)
...
const runAll = async () => {
  ...
  const pending = files.value.filter(f => !f.result)
