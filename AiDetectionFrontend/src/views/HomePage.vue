        ...
        <div class="home-page__controls">
          ...
          <div v-else class="local-mode-summary">
            ...
            <span>
              ...
              <span class="local-mode-summary__text">Вероятность ИИ/человек и фрагменты для длинного текста</span>
            </span>
          </div>
...
const providerOptions: ProviderOption[] = [
  ...
  },
  {
    ...
    icon: 'shield',
    hint: 'Пилотная локальная модель: оконный анализ текста, общий результат и вероятности по фрагментам.',
  },
]
...
const activeDepthHint = computed(() =>
  isLocalProvider.value
    ? 'Локальная модель работает на вашем сервере, режет длинный текст на окна и показывает вероятность ИИ по фрагментам. Модельная атрибуция LLM доступна только в расширенном облачном режиме.'
    : depth.value === 'extended'
